...
def add_execution_section(doc: Document):
    ...
    add_bullet(doc, "установка серверных зависимостей командой `pip install -r backend/requirements.txt`;")
    add_bullet(doc, "переход в каталог `floorplan-ui` и установка клиентских зависимостей командой `npm install`;")
    ...
    add_paragraph(
        doc,
        "Для запуска клиентской части оператор переходит в каталог `floorplan-ui` и "
