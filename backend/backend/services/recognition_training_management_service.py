...
class RecognitionTrainingManagementService:
    ...
    def get_run_log(self, run_id: str, *, tail: int = 200) -> dict[str, Any]:
        ...
        log_asset_path = self._storage_asset_path(run.log_path)
        if log_path is None or not log_path.exists():
            return {
                "run_id": run_id,
                "log_path": log_asset_path,
                "log_url": build_asset_url(log_asset_path),
                "content": "",
            }
    ...
    def _serialize_run(
        ...
        *,
        ...
        payload = run.to_dict()
        payload["log_path"] = self._storage_asset_path(run.log_path)
        ...
        payload["log_url"] = build_asset_url(payload["log_path"])
    ...
    def _serialize_active_model(self, active_model: RecognitionActiveModel | None) -> dict[str, Any] | None:
        ...
        payload["artifact_path"] = self._storage_asset_path(active_model.artifact_path) or active_model.artifact_path
    ...
    def _storage_asset_path(self, path_value: str | None) -> str | None:
        resolved = self._resolve_storage_path(path_value)
        if resolved is None:
            return None
        try:
            return self._storage_relative_path(resolved)
        except ValueError:
            return None
