...
from backend.models import FloorPlan, Project, ProjectPdfVersion
...
class BackgroundTaskExecutor:
    ...
    def execute(self, task_id: int) -> dict:
        ...
        task_service.update_progress(task.id, percent=10, stage="running")
        ...
        task_service.update_progress(task.id, percent=90, stage="finalizing")
    ...
    def _execute_task(
        ...
        *,
        ...
        if task_type == TASK_PROJECT_PDF_GENERATE:
            return self._execute_project_pdf_generate(
                payload,
                task_id=task_id,
    ...
    def _execute_project_pdf_generate(
        self,
        payload: dict,
        *,
        task_id: int,
        ...
        version = ProjectPdfVersion(
            project_id=project_id,
            path=stored_pdf_path,
            generated_by_user_id=requested_by_user_id,
            background_task_id=task_id,
            params={key: value for key, value in payload.items() if key != "project_id"},
