...
def _default_data_dir() -> Path:
    """Prefer pre-move runtime data when this repo has just been regrouped."""
    if REPOSITORY_ROOT != PROJECT_ROOT and (REPOSITORY_ROOT / "floor_plans.db").exists():
        return REPOSITORY_ROOT
    return PROJECT_ROOT


DEFAULT_DATA_DIR = _env_path("DATA_DIR", _default_data_dir())
...
class AppConfig:
    ...
    min_floor_plan_image_size_px: int = int(os.getenv("MIN_FLOOR_PLAN_IMAGE_SIZE_PX", "200"))
    allowed_image_types: tuple[str, ...] = tuple(_env_csv("ALLOWED_IMAGE_TYPES", ["png", "jpg", "jpeg", "webp", "jfif"]))
