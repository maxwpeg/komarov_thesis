    ...
    with engine.begin() as connection:
        ...
        if "background_tasks" in table_names:
            ...
            if "progress_percent" not in task_columns:
                connection.exec_driver_sql("ALTER TABLE background_tasks ADD COLUMN progress_percent INTEGER DEFAULT 0 NOT NULL")
            if "progress_stage" not in task_columns:
                connection.exec_driver_sql("ALTER TABLE background_tasks ADD COLUMN progress_stage VARCHAR(120)")
        if "audit_events" in table_names:
            ...
            if "user_id" not in audit_columns:
                connection.exec_driver_sql("ALTER TABLE audit_events ADD COLUMN user_id INTEGER")
...
def _ensure_performance_indexes() -> None:
    ...
    statements = (
        ...
        "CREATE INDEX IF NOT EXISTS ix_background_tasks_progress ON background_tasks (progress_percent, progress_stage)",
        "CREATE INDEX IF NOT EXISTS ix_project_pdf_versions_project_created ON project_pdf_versions (project_id, created_at)",
        "CREATE INDEX IF NOT EXISTS ix_project_pdf_versions_background_task ON project_pdf_versions (background_task_id)",
