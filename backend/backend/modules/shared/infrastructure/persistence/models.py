...
class Project(Base):
    ...
    pdf_versions = relationship("ProjectPdfVersion", back_populates="project", cascade="all, delete-orphan")
...
class BackgroundTask(Base):
    ...
    progress_stage = Column(String(120), nullable=True)
...
class ProjectPdfVersion(Base):
    """Generated PDF version for a project."""

    __tablename__ = "project_pdf_versions"
    ...
    project = relationship("Project", back_populates="pdf_versions")
    generated_by_user = relationship("User")
    background_task = relationship("BackgroundTask")

    def to_dict(self):
        generated_by = self.generated_by_user.to_summary_dict() if self.generated_by_user is not None else None
        return {
            "id": self.id,
            "project_id": self.project_id,
            "path": self.path,
