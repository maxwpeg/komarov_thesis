...
class SqlAlchemyProjectRepository(ProjectRepository):
    ...
    def list(
        ...
        if normalized_status in {"active", "open"}:
            query = query.filter(ProjectModel.deleted_at.is_(None))
        elif normalized_status in {"deleted", "trash"}:
            query = query.filter(ProjectModel.deleted_at.is_not(None))
        elif normalized_status:
            query = query.filter(ProjectModel.stage == status)
        if project_number is not None:
            query = query.filter(ProjectModel.number == project_number)
        normalized_facility = (facility or "").strip()
        if normalized_facility:
            query = query.filter(ProjectModel.facility.ilike(f"%{normalized_facility}%"))
        ...
        if normalized_search:
            ...
            query = query.filter(
                or_(
                    ...
                    ProjectModel.facility.ilike(pattern),
                    ProjectModel.code.ilike(pattern),
                    ProjectModel.contractor.ilike(pattern),
                    ProjectModel.engineer.ilike(pattern),
                )
            )
        if date_from is not None:
            query = query.filter(ProjectModel.created_at >= date_from)
        if date_to is not None:
            query = query.filter(ProjectModel.created_at <= date_to)
