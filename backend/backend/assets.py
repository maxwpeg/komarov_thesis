...
def _legacy_absolute_asset_path(raw: str) -> str | None:
    candidate = Path(raw)
    ...
    roots = (
        ...
        (settings.outputs_dir.resolve(), "outputs"),
        (settings.debug_output_dir.resolve(), "debug_output"),
    )
    ...
    raise ValueError("Absolute asset path must point inside managed storage")
...
def normalize_asset_path(value: str | None) -> str | None:
    ...
    raw = _legacy_absolute_asset_path(raw_value)
    if raw is None:
