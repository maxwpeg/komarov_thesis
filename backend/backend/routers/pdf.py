...
from backend.mappers import (
    ...
    project_pdf_version_read,
    power_consumption_calculation_read,
)
from backend.models import BackgroundTask, Project, ProjectPdfVersion
...
def get_project_pdf_preview(
    ...
    versions = (
        db.query(ProjectPdfVersion)
        .filter(ProjectPdfVersion.project_id == project_id)
        .order_by(ProjectPdfVersion.created_at.desc(), ProjectPdfVersion.id.desc())
        .limit(50)
        .all()
    )
    ...
    return ProjectPdfPreviewRead(
        ...
        versions=[project_pdf_version_read(version) for version in versions],
    )


@router.get("/api/projects/{project_id}/pdf-versions", response_model=list[ProjectPdfVersionRead])
def list_project_pdf_versions(
    project_id: int,
    _: AuthenticatedUser = Depends(require_project_access),
    db: Session = Depends(get_db),
) -> list[ProjectPdfVersionRead]:
    versions = (
        db.query(ProjectPdfVersion)
        .filter(ProjectPdfVersion.project_id == project_id)
        .order_by(ProjectPdfVersion.created_at.desc(), ProjectPdfVersion.id.desc())
    ...
    return [project_pdf_version_read(version) for version in versions]
