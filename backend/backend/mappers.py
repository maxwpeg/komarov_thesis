...
def project_pdf_version_read(version: ProjectPdfVersion) -> schemas.ProjectPdfVersionRead:
    payload = _payload(version)
    payload["url"] = build_asset_url(payload.get("path"))
    return schemas.ProjectPdfVersionRead.model_validate(payload)


def audit_event_read(event: AuditEvent) -> schemas.AuditEventRead:
    return schemas.AuditEventRead.model_validate(_payload(event))
