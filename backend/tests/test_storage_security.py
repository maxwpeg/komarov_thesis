...
def _png_bytes() -> bytes:
    buffer = io.BytesIO()
    Image.new("RGB", (240, 240), color="white").save(buffer, format="PNG")
...
def test_upload_validation_rejects_wrong_extension_and_fake_image(api_server: str):
    ...
    small_image = requests.post(
        f"{api_server}/api/floor-plans",
        data={"project_id": str(project["id"]), "floor_number": "1"},
        files={"file": ("floor.png", _small_png_bytes(), "image/png")},
        timeout=10,
    )
    assert small_image.status_code == 400
    assert small_image.json()["code"] == "invalid_upload_file"
