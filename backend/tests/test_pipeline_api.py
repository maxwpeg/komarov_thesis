...
def wait_background_task(base_url: str, task: dict, timeout_seconds: float = 15.0) -> dict:
    ...
    while time.monotonic() < deadline:
        if current["status"] in {"succeeded", "failed", "canceled"}:
            break
        time.sleep(0.2)
        response = requests.get(f"{base_url}/api/background-tasks/{task_id}", timeout=10)
        assert response.status_code == 200
...
def run_pipeline_detect(base_url: str, floor_plan_id: int, step: str) -> dict:
    response = requests.post(
        f"{base_url}/api/floor-plans/{floor_plan_id}/pipeline/{step}/detect",
        timeout=10,
    )
    if response.status_code == 200:
        return response.json()
    assert response.status_code == 202
    wait_background_task(base_url, response.json())
    floor_plan_response = requests.get(f"{base_url}/api/floor-plans/{floor_plan_id}?include_elements=true", timeout=10)
    assert floor_plan_response.status_code == 200
    state_response = requests.get(f"{base_url}/api/floor-plans/{floor_plan_id}/pipeline-state", timeout=10)
    assert state_response.status_code == 200
    return {
        "floor_plan": floor_plan_response.json(),
        "pipeline_state": state_response.json(),
    }
...
def test_pipeline_room_detection_keeps_perimeter_room_with_small_outer_gap(api_server: str, tmp_path: Path):
    ...
    payload = run_pipeline_detect(api_server, floor_plan["id"], "rooms")
...
def test_zkspc_detect_and_commit_unlock_branch_steps(api_server: str):
    ...
    detected_payload = run_pipeline_detect(api_server, floor_plan["id"], "zkspc")
...
def test_branch_specific_fire_alarm_saves_do_not_overwrite_other_system(api_server: str):
    ...
    detected_payload = run_pipeline_detect(api_server, floor_plan["id"], "zkspc")
    zkspc_zone_id = detected_payload["floor_plan"]["zkspc_zones"][0]["id"]
...
def test_step_feedback_routes_capture_wall_and_opening_samples(api_server: str, tmp_path: Path):
    ...
    walls_payload = run_pipeline_detect(api_server, floor_plan["id"], "walls")
    assert walls_payload["pipeline_state"]["steps"]["walls"]["status"] == "draft"
    ...
    openings_payload = run_pipeline_detect(api_server, floor_plan["id"], "openings")
    assert openings_payload["pipeline_state"]["steps"]["openings"]["status"] == "draft"
