...
export default function WorkerTasksPage() {
  ...
  const handleRetryTask = async (taskId) => {
    ...
    setError('');
    try {
      await backgroundTasksApi.retry(taskId);
      await loadTasks({ silent: true });
      toast(`Задача #${taskId} снова поставлена в очередь.`, { tone: 'info' });
    } catch (retryError) {
      console.error('Error retrying worker task:', retryError);
            ...
            <tbody>
              {tasks.map((task) => (
                <tr key={task.id}>
                  ...
                  <td>
                    <div className="worker-task-progress" title={task.progress_stage || ''}>
                      <div className="worker-task-progress__track">
                        <span style={{ width: `${Math.max(0, Math.min(Number(task.progress_percent || 0), 100))}%` }} />
                      </div>
                      <small>{Math.max(0, Math.min(Number(task.progress_percent || 0), 100))}% {task.progress_stage || ''}</small>
                    </div>
                  </td>
                  ...
                  <td>
                    {task.status === 'queued' ? (
                      ...
                      >
                        {retryingTaskId === task.id ? 'Повтор...' : 'Повторить'}
                      </button>
                    ) : '—'}
                  </td>
                </tr>
