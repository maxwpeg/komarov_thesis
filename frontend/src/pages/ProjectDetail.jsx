...
const handleDeleteFloorPlan = async (event, floorPlanId, floorPlanName) => {
  ...
  const pdfVersions = pdfPreview?.versions || [];
  const pdfTaskProgress = Math.max(0, Math.min(Number(pdfPreview?.current_task?.progress_percent || 0), 100));
    ...
    <div className="project-detail-page">
      <div className="project-list-container project-detail-page__panel">
          ...
          <div className="project-detail-toolbar">
            {isPdfPreparing ? (
              <div className="project-detail-toolbar__status">
                PDF поставлен в очередь и готовится в фоне: {pdfTaskProgress}% {pdfPreview?.current_task?.progress_stage || ''}
                <div className="worker-task-progress__track project-pdf-progress">
                  <span style={{ width: `${pdfTaskProgress}%` }} />
                </div>
              </div>
            ) : (
              <div className="project-detail-toolbar__status">
        ...
        {pdfVersions.length > 0 && (
          <section className="project-pdf-versions">
            <div className="project-pdf-versions__header">
              <h2>Версии PDF</h2>
            </div>
              ...
              <table className="training-table">
                ...
                <tbody>
                  {pdfVersions.map((version) => (
                    <tr key={version.id}>
                      <td>{version.created_at ? new Date(version.created_at).toLocaleString('ru-RU') : '—'}</td>
                      <td>{version.generated_by_user?.full_name || version.generated_by_user_id || '—'}</td>
                      <td>{version.params && Object.keys(version.params).length ? JSON.stringify(version.params) : '—'}</td>
                      <td>
                        {version.url ? (
                          <a href={version.url} target="_blank" rel="noopener noreferrer">Скачать</a>
                        ) : '—'}
                      </td>
                    </tr>
                  ))}
