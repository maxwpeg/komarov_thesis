...
function ProjectList() {
  ...
  const handleDeleteProject = async (e, projectId, projectName) => {
    ...
    if (!window.confirm(`Вы уверены, что хотите удалить проект "${projectName}"? Это действие необратимо.`)) {
      return;
    }
        ...
        <div className="project-grid">
          {projects.map((project) => (
            <div key={project.id} style={{ position: 'relative' }}>
              ...
              <button
                onClick={(e) => handleDeleteProject(e, project.id, project.facility)}
                style={{
                  position: 'absolute',
                  top: '10px',
                  right: '10px',
